...
# Incomplete Branches
  * [EPT]() *\[Forked from GuestState\]* - Simplistic EPT configuration supporting (only) 4KB guest pages, designed to only virtualize the required guest memory. Complete with memory management helper routines, this branch also demonstrates modifications to the underlying EPT tables (in addition to splitting attacks) to both modify and hide memory exposed to the guest.
  * [EPTIdentity]() *\[Forked from EPT\]* - EPT configuration designed to support 2MB large pages in an guest-to-host identity map (full system memory virtualization). Also demonstrates EPT splitting attacks by selectively splitting target 2MB pages to their 4KB equivalents.
  * [EventInjection]() *\[Forked from GuestState\]* - Demonstrates the simulation of VMX instructions outside VMX operation while within VMX operation. Additionally, we also demonstrate emulating the `INVD` and `WBINVD` instructions, with explanations on how could go about building such a feature in for other instructions which cause non-conditional VM-exits.
  * [BluePill]() *\[Forked from GuestState\]* - Performs the widely publicized *Blue Pill* rootkit attack by Joanna Rutkowska. Also known as *hyper-jacking*, this attack attempts to virtualize (as a type-2 hypervisor) the underlying operating system in order to run the VMM within a more privileged space than the operating system itself. This hypervisor is designed to only support the minimum-possible VM-exit conditions, and will modify the return result of a `CPUID` instruction. As a final design touch, it will implement Virtual Processor IDs (VPIDs) for a performance benefit.
