...
The modifications to guest behavior facilitated by EPT in this branch mostly operate on the basis of functions. It probably would have been a more simplistic idea to implement the behavioral modifications demonstrated by accessing variables in a data page exposed to the guest, but I wanted to use functions instead. To do this, I placed each of the functions in their own PE section. This ensured that each of the code chunks would be inserted in individual allocations of at least 4KB, isolating these functions to their own pages; and effectively nullifying any risk of splitting these functions between different pages, considering their small size. Once the guest begins executing, it executs a `HLT` instruction, breaking back into the VMM, where one of the EPT PTEs pointing to a function, `GuestTargetFn`, is replaced with a PTE that points to a different function, `GuestHookFn`. This results in execution being redirected to the `GuestHookFn`, as seen in the screenshot below.
...
A closing note regarding the paging translation mechanism: sometimes the processor won't go through this whole process to translate a virtual address to a physical one. After a virtual address is translated to a physical one, often times this address is cached in a buffer within the CPU called the Translation Lookaside Buffer (TLB). As you can guess, this allows the processor to more quickly perform virtual to physical address translations for addresses that are repeatedly used to access memory. For this reason, if you were to modify the paging tables to manipulate a processes virtual address space, it's very possible you would additionally have to invalidate the TLB. This problem is regularly encountered with modifying EPT tables, and has an instruction devoted to solving this problem called `INVEPT`.

One last thing before we leave off: there is a concept called *page faults* which occur when the CPU attempts to access a paging entry which does not exist—or is not otherwise present. This could be an entry that has been paged out to disk, as talked about above, but this could also be (in our case) an entry that doesn't have the relevant page table entries to complete a virtual address translation. You should understand the idea of page faults thoroughly before embarking on a journey with EPT, as you will most certainly encounter another concept called *EPT violations*, which can indicate page faults in EPT translations. To read more about this, you can check out volume 3A of the SDM, section 4.7, "Page-Fault Exceptions".
...
Most projects you will see do not setup their EPT to reflect such granular allocations as we have in this project. Instead, they'll perform the same sort of identity mapping with the host that I talked about above, but with special *large pages*. One of the bits in the paging table entries allows for a behavioral modification to the translation mechanism in which tables can, instead of indexing tables beneath them, index one large page allocation (mapping actual data) to represent the same amount of memory that would present if the underlying tables were completely filled with mappings. Allow me to clarify. A PT can hold 512 8-byte entries. Since each of the PTEs can map one 4KB data allocation, we have a total of 2MB worth of memory (`512 * 4096`) that is capable of being represented by one PT. Setting the large page bit on a PDE—which, as we know from earlier, points to the base of a PT—allows us to use the address pointed to by the PDE as one large 2MB individual page, instead of indexing a PT which is capable of indexing the same amount of data, just in individual 4KB allocations. This same technique can be used with PDPTs indexing PDEs to create a large page entry capable of mapping 1GB of data (rather than 512 PDTs). You would only really want to use a technique like this if you were mapping large *ranges* of memory. As such, hypervisors attempting to identity map all of the physical memory on a machine will commonly utilize these large pages, perhaps along with something like the underlying OS memory maps or outside MTRRs. This technique can be seen in the *EPTIdentity* branch in this repository.

In order to further understand EPT, and the steps that are required to implement EPT, I highly recommend—like in the previous branches—that you read the chapters out of the Intel SDM that have been diligently placed as comments next to code throughout this project. More specifically, you should read through each of the chapters cited in the `EPT.h` header file; most importantly volume 3C, section 28.2, "The Extended Page Table Mechanism (EPT)". Around this reading, you will also want to checkout the aforementioned concept of EPT violations.

# Changes From the GuestState Branch
  - Several changes have been made to the `DriverEntry` function used by the hypervisor. These changes provide checks on VMCS controls (such as processor secondary controls), and other required virtualization features (such as EPT support) used to demonstrate EPT usage. Code has also been inserted to call a function, `EptBuild`, which is responsible for initializing our EPT pointer (a pointer to the PML4 base address). Finally, some error checking code has been inserted at VM launch so that you can more easily detect errors with the addition of EPT.
  - **EPT.h/c** source files contain functions designed to facilitate the creation of the EPT, along with helper functions which can be used to insert virtual addresses into the EPT as a series of identity-mapped guest-physical address.
  - **Memory.h/c** these source files contain one helper function, designed to obtain all of the page table physical base addresses for a given virtual address. 

A quick note for those following along with these branches, or attempting to implement granular EPT entries: watch your entries. To only run a very simple snippet of guest source code, the target function and the stack are the only required EPT entries. However, understand that, with a 24KB guest stack, you will have to insert each of 4KB pages used by the stack. This was a *gotcha* for me, as initially I was only inserting the first 4KB page of stack, whereas I needed to be inserting the last 4KB allocation of the stack. You can catch errors like this by closely inspecting the EPT-relevant VM-exit information fields, seen heavily commented in my source [here](https://github.com/calware/HV-Playground/blob/EPT/SPTHv/Handler.c#L41), while correlating that data with our guest context.
